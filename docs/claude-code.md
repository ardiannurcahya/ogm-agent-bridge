...
Clone bridge, run `uv sync --locked`, then copy `examples/claude-code/.mcp.json.example` to trusted project `.mcp.json`. Replace `/absolute/path/ogm-agent-bridge` with clone path.
...
Verify with `claude mcp list`, then call `ogm_health` and `ogm_search_entities`. `read-only` blocks upload.
