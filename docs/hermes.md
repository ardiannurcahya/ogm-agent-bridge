...
Clone bridge, run `uv sync --locked`, and merge `examples/hermes/config.yaml.example` under `mcp_servers`. Replace `/absolute/path/ogm-agent-bridge` with clone path.
...
Run `hermes mcp test ogm`, then call health and entity search. `read-only` blocks upload.
